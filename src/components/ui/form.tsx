...
export function Field({
  ...
  hint?: string;
  required?: boolean;
  /** Keep the label for screen readers only — for placeholder-led layouts. */
  hideLabel?: boolean;
  surface?: Surface;
  className?: string;
      ...
      {label ? (
        <label
          ...
          className={cn(
            hideLabel
              ? "sr-only"
              : cn("text-body-sm", surface === "light" ? "text-ink/70" : "text-white/70")
          )}
        >
          {label}
          {required ? (
...
export function Select({
  ...
  return (
    <div className="relative">
      ...
      {/* Native chevron is removed by appearance-none — this fills the reserved pr-10. */}
      <ChevronDown
        aria-hidden
        className={cn(
          "pointer-events-none absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2",
          surface === "light" ? "text-ink/45" : "text-white/45"
        )}
      />
    </div>
  );
